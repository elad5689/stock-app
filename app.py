...
# CSS יוקרתי (החזרת העיצוב המקורי)
st.markdown("""
    <style>
...
def load_data(symbol):
    try:
        # הורדה עם מנגנון עקיפת חסימות משופר
        ticker_obj = yf.Ticker(symbol)
        # שימוש ב-history עם הוראה מפורשת לסינכרון
...
if not data.empty and len(data) > 10:
    ...
    fig.update_xaxes(range=[start_date, end_date], gridcolor='#1e293b')
    ...
    st.info(f"Connecting to markets for {ticker}... If this persists, try refreshing the page.")
    # כפתור רענון ידני ל-Cache
