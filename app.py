...
opts = {
    ...
    "SMA 9": st.sidebar.toggle("SMA 9", value=True), # נוסף כאן
...
def load_data(symbol):
    try:
        t = yf.Ticker(symbol)
        df = t.history(period="2y") # הגדלתי ל-2y כדי שיהיה מספיק מידע ל-SMA 200
        if df.empty:
...
if not data.empty and len(data) > 0:
    ...
    data['SMA9'] = data['Close'].rolling(9).mean() # חישוב SMA 9
