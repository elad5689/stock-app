...
# CSS - הוספת עיצוב ספציפי לכפתור ה-Fintel
st.markdown("""
    <style>
    .stApp {
    ...
    header[data-testid="stHeader"] {
        background: linear-gradient(90deg, #0a192f 0%, #000000 100%) !important;
    ...
    h1, h2, h3, h4, span, label, p, .stCheckbox, .stToggleButton { 
...
show_avwap = st.sidebar.toggle("AVWAP", value=True)
show_sma200 = st.sidebar.toggle("SMA 200", value=True)
show_sma50 = st.sidebar.toggle("SMA 50", value=True)
show_sma20 = st.sidebar.toggle("SMA 20", value=True)
show_ema20 = st.sidebar.toggle("EMA 20", value=True)
show_ema5 = st.sidebar.toggle("EMA 5", value=True)
...
if not data.empty:
    ...
    colors = {'SMA200': '#ff1744', 'SMA50': '#00e676', 'SMA20': '#ff9100', 'EMA20': '#00e5ff', 'EMA5': '#007bff'}
    
    if show_sma200: fig.add_trace(go.Scatter(x=data.index, y=data['SMA200'], line=dict(color=colors['SMA200'], width=2), name='SMA 200'), row=1, col=1)
    if show_sma50: fig.add_trace(go.Scatter(x=data.index, y=data['SMA50'], line=dict(color=colors['SMA50'], width=1.5), name='SMA 50'), row=1, col=1)
    if show_sma20: fig.add_trace(go.Scatter(x=data.index, y=data['SMA20'], line=dict(color=colors['SMA20'], width=1.5), name='SMA 20'), row=1, col=1)
    if show_ema20: fig.add_trace(go.Scatter(x=data.index, y=data['EMA20'], line=dict(color=colors['EMA20'], width=1.3), name='EMA 20'), row=1, col=1)
    if show_ema5: fig.add_trace(go.Scatter(x=data.index, y=data['EMA5'], line=dict(color=colors['EMA5'], width=1.3), name='EMA 5'), row=1, col=1)
    ...
    fig.update_xaxes(
        gridcolor='#1e293b', tickfont=dict(color="white"),
    ...
    fig.update_layout(
        height=650, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='#000000',
        xaxis_rangeslider_visible=False, margin=dict(l=10, r=10, t=10, b=10), 
        hovermode="x unified", legend=dict(font=dict(color="white", size=11)),
